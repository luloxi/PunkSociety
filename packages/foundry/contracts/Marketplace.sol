...
contract Marketplace is ReentrancyGuard, Ownable {
  ...
  event Purchase(
    uint256 indexed itemId, address buyer, Currency currency, uint256 price
  );
  ...
  function buy(uint256 listingId) public payable nonReentrant {
    ...
    if (payableCurrency == Currency.USDCToken) {
      //   bool usdcSuccess = IERC20(USDC).transferFrom(
      //     msg.sender, nftSeller, price - ((price * baseRoyalty) / BPS)
      //   ); // price - royalty
      bool usdcSuccess = IERC20(USDC).transferFrom(msg.sender, nftSeller, price); // price - royalty
      // Transfer royalty to the royalty receiver
      //   bool usdcRoyaltiesSuccess = IERC20(USDC).transferFrom(
      //     msg.sender, royaltyReceiver, (price * baseRoyalty) / BPS
      //   ); // just royalty
      //   require(usdcSuccess && usdcRoyaltiesSuccess, "Failed to transfer USDC");
      require(usdcSuccess, "Failed to transfer USDC");
    } else {
      require(
        msg.value >= price, "Please submit asking price to complete purchase"
      );
      //   (bool ethSuccess,) = payable(nftSeller).call{
      //     value: price - ((price * baseRoyalty) / BPS)
      //   }("");
      (bool ethSuccess,) = payable(nftSeller).call{ value: price }("");
      //   (bool ethRoyaltiesSuccess,) =
      //     payable(royaltyReceiver).call{ value: (price * baseRoyalty) / BPS }("");
      //   require(ethSuccess && ethRoyaltiesSuccess, "Failed to transfer ETH");
      require(ethSuccess, "Failed to transfer ETH");
    }
  ...
  function updatePrice(
    uint256 listingId,
    uint256 newPrice
  ) public Owned(listingId) {
    require(
      !listings[listingId].isAuction,
      "Auction starting prices cannot be updated"
    );
  ...
  function bid(uint256 listingId, uint256 amount) public payable nonReentrant {
    ...
    require(
      msg.sender != listing.seller, "You cannot bid on an NFT you are selling."
    );
    require(listing.isAuction, "This listing is not an auction");
    require(block.timestamp < listing.date, "Auction has already ended");
    require(
      amount > listing.price, "Bid must be greater than the previous bid."
    );
  ...
  function withdraw(uint256 listingId) public nonReentrant {
    ...
    require(
      msg.sender == nftSeller || msg.sender == highestBidder,
      "Only the seller or highest bidder can withdraw."
    );
  ...
  function setNFTCollectionRoyalty(
    ...
    royalties[contractAddress] =
      Royalties(contractAddress, payoutAccount, royaltyAmount, true);
